...
// Build SQL query - ONLY Club category
$sql = "SELECT po.*, ";
if ($is_logged_in) {
...
$sql .= "FROM pathways_opportunities po WHERE po.category = 'Club'";
...
$sql .= " ORDER BY po.state, po.program_name LIMIT ? OFFSET ?";
...
$count_sql = "SELECT COUNT(*) as total FROM pathways_opportunities po WHERE po.category = 'Club'";
$count_params = [];
$count_types = "";
...
if (!empty($search_field)) {
    $count_sql .= " AND po.field = ?";
    $count_params[] = $search_field;
    $count_types .= "s";
}
...
$fields_query = "SELECT DISTINCT field FROM pathways_opportunities WHERE category = 'Club' AND field IS NOT NULL ORDER BY field";
  ...
  <section class="search-section">
    <div class="container">
      ...
      <div class="search-header" data-aos="fade-down">
        <h1><i class="bi bi-people"></i> School Clubs & Organizations</h1>
        <p>Discover clubs and organizations to join at your school</p>
      </div>
        ...
        <form method="GET" action="">
          <div class="filter-row">
            <div class="form-group">
              <label for="keyword">Search Clubs</label>
              <input type="text" id="keyword" name="keyword" placeholder="Search by club name, field, or description..." value="<?php echo htmlspecialchars($search_keyword); ?>">
            </div>
            ...
            <div class="form-group">
              ...
              <select id="field" name="field">
                ...
                <?php while ($field = $fields_result->fetch_assoc()): ?>
                  <option value="<?php echo htmlspecialchars($field['field']); ?>" <?php echo ($search_field === $field['field']) ? 'selected' : ''; ?>>
          ...
          <?php if (!empty($search_keyword) || !empty($search_state) || !empty($search_field) || !empty($search_deadline) || !empty($search_grade)): ?>
          <div style="margin-top: 15px;">
            <a href="clubs.php" class="btn-clear">
      ...
      <div class="results-info">
        <div>
          <strong><?php echo $total_results; ?></strong> clubs found
        ...
        <div style="font-size: 14px; color: #666;">
          <i class="bi bi-info-circle"></i> <a href="login-page.php" style="color: #5fcf80; text-decoration: none;">Login</a> to save favorites
        </div>
        <?php endif; ?>
      </div>
      ...
      <?php if ($total_pages > 1): ?>
        <div class="pagination">
          <?php if ($page > 1): ?>
            <a href="?keyword=<?php echo urlencode($search_keyword); ?>&state=<?php echo urlencode($search_state); ?>&field=<?php echo urlencode($search_field); ?>&deadline=<?php echo urlencode($search_deadline); ?>&grade=<?php echo urlencode($search_grade); ?>&page=<?php echo ($page - 1); ?>">
          ...
          if ($start_page > 1): ?>
            <a href="?keyword=<?php echo urlencode($search_keyword); ?>&state=<?php echo urlencode($search_state); ?>&field=<?php echo urlencode($search_field); ?>&deadline=<?php echo urlencode($search_deadline); ?>&grade=<?php echo urlencode($search_grade); ?>&page=1">1</a>
            <?php if ($start_page > 2): ?>
              <span>...</span>
            <?php endif; ?>
          <?php endif; ?>
          ...
          <?php for ($i = $start_page; $i <= $end_page; $i++): ?>
            ...
            <?php else: ?>
              <a href="?keyword=<?php echo urlencode($search_keyword); ?>&state=<?php echo urlencode($search_state); ?>&field=<?php echo urlencode($search_field); ?>&deadline=<?php echo urlencode($search_deadline); ?>&grade=<?php echo urlencode($search_grade); ?>&page=<?php echo $i; ?>"><?php echo $i; ?></a>
            <?php endif; ?>
          <?php endfor; ?>
          ...
          <?php if ($end_page < $total_pages): ?>
            <?php if ($end_page < $total_pages - 1): ?>
              <span>...</span>
            <?php endif; ?>
            <a href="?keyword=<?php echo urlencode($search_keyword); ?>&state=<?php echo urlencode($search_state); ?>&field=<?php echo urlencode($search_field); ?>&deadline=<?php echo urlencode($search_deadline); ?>&grade=<?php echo urlencode($search_grade); ?>&page=<?php echo $total_pages; ?>"><?php echo $total_pages; ?></a>
          <?php endif; ?>
