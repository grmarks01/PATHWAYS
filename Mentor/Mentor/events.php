...
// Build SQL query - ONLY Competition, Academic Program, and Program categories
$sql = "SELECT po.*, ";
if ($is_logged_in) {
...
$sql .= "FROM pathways_opportunities po WHERE po.category IN ('Competition', 'Academic Program', 'Program')";
...
if (!empty($search_keyword)) {
    $sql .= " AND (po.program_name LIKE ? OR po.notes LIKE ? OR po.field LIKE ?)";
...
$count_sql = "SELECT COUNT(*) as total FROM pathways_opportunities po WHERE po.category IN ('Competition', 'Academic Program', 'Program')";
$count_params = [];
$count_types = "";

if (!empty($search_keyword)) {
    $count_sql .= " AND (po.program_name LIKE ? OR po.notes LIKE ? OR po.field LIKE ?)";
    $keyword_param = "%$search_keyword%";
...
<head>
  ...
  <title>Events & Programs - Pathways</title>
  ...
  <section class="search-section">
    <div class="container">
      ...
      <div class="search-header" data-aos="fade-down">
        <h1><i class="bi bi-calendar-event"></i> Events, Competitions & Programs</h1>
        <p>Discover competitions, academic programs, and special events</p>
      </div>
        ...
        <form method="GET" action="">
          <div class="filter-row">
            <div class="form-group">
              ...
              <input type="text" id="keyword" name="keyword" placeholder="Search by event name, field, or description..." value="<?php echo htmlspecialchars($search_keyword); ?>">
            </div>
            ...
            <div class="form-group">
              ...
              <select id="category" name="category">
                ...
                <option value="Competition" <?php echo ($search_category === 'Competition') ? 'selected' : ''; ?>>Competition</option>
                <option value="Academic Program" <?php echo ($search_category === 'Academic Program') ? 'selected' : ''; ?>>Academic Program</option>
                <option value="Program" <?php echo ($search_category === 'Program') ? 'selected' : ''; ?>>Program</option>
              </select>
            </div>
      ...
      <div class="results-info">
        <div>
          <strong><?php echo $total_results; ?></strong> events found
        ...
        <div style="font-size: 14px; color: #666;">
          <i class="bi bi-info-circle"></i> <a href="login-page.php" style="color: #667eea; text-decoration: none;">Login</a> to save favorites
        </div>
        <?php endif; ?>
      </div>
        ...
        <?php foreach ($opportunities as $opp): ?>
          <div class="opportunity-card" data-aos="fade-up">
            ...
            <div class="opportunity-badges">
              ...
              $badge_class = 'badge-competition';
              if ($opp['category'] === 'Academic Program') {
                  $badge_class = 'badge-academic';
              } elseif ($opp['category'] === 'Program') {
                  $badge_class = 'badge-program';
              }
              ?>
              <span class="badge <?php echo $badge_class; ?>"><?php echo htmlspecialchars($opp['category']); ?></span>
              <span class="badge badge-state"><?php echo htmlspecialchars($opp['state']); ?></span>
              <?php if ($opp['field']): ?>
                <span class="badge badge-field"><?php echo htmlspecialchars($opp['field']); ?></span>
              <?php endif; ?>
            </div>
            ...
            <div class="opportunity-description">
              <?php if ($opp['eligibility']): ?>
                <p><strong>Eligibility:</strong> <?php echo htmlspecialchars($opp['eligibility']); ?></p>
              <?php endif; ?>
              <?php if ($opp['notes']): ?>
                <p><strong>Notes:</strong> <?php echo htmlspecialchars($opp['notes']); ?></p>
              <?php endif; ?>
            </div>
            <?php endif; ?>
      ...
      <?php if ($total_pages > 1): ?>
        <div class="pagination">
          <?php if ($page > 1): ?>
            <a href="?keyword=<?php echo urlencode($search_keyword); ?>&state=<?php echo urlencode($search_state); ?>&category=<?php echo urlencode($search_category); ?>&deadline=<?php echo urlencode($search_deadline); ?>&grade=<?php echo urlencode($search_grade); ?>&page=<?php echo ($page - 1); ?>">
          ...
          if ($start_page > 1): ?>
            <a href="?keyword=<?php echo urlencode($search_keyword); ?>&state=<?php echo urlencode($search_state); ?>&category=<?php echo urlencode($search_category); ?>&deadline=<?php echo urlencode($search_deadline); ?>&grade=<?php echo urlencode($search_grade); ?>&page=1">1</a>
            <?php if ($start_page > 2): ?>
              <span>...</span>
            <?php endif; ?>
          <?php endif; ?>
          ...
          <?php for ($i = $start_page; $i <= $end_page; $i++): ?>
            ...
            <?php else: ?>
              <a href="?keyword=<?php echo urlencode($search_keyword); ?>&state=<?php echo urlencode($search_state); ?>&category=<?php echo urlencode($search_category); ?>&deadline=<?php echo urlencode($search_deadline); ?>&grade=<?php echo urlencode($search_grade); ?>&page=<?php echo $i; ?>"><?php echo $i; ?></a>
            <?php endif; ?>
          <?php endfor; ?>
          ...
          <?php if ($end_page < $total_pages): ?>
            <?php if ($end_page < $total_pages - 1): ?>
              <span>...</span>
            <?php endif; ?>
            <a href="?keyword=<?php echo urlencode($search_keyword); ?>&state=<?php echo urlencode($search_state); ?>&category=<?php echo urlencode($search_category); ?>&deadline=<?php echo urlencode($search_deadline); ?>&grade=<?php echo urlencode($search_grade); ?>&page=<?php echo $total_pages; ?>"><?php echo $total_pages; ?></a>
          <?php endif; ?>
